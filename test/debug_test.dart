...
void main() async {
  ...
  print(
      '   type=$rType body=${rBody.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}');
    ...
    while (pos < body.length) {
      ...
      if (tok == 0xFD || tok == 0xFE || tok == 0xFF) {
        ...
        final msg = String.fromCharCodes([
          for (int j = 0; j < msgBytes.length; j += 2)
            msgBytes[j] | (msgBytes[j + 1] << 8)
        ]);
        print(
            '     ${tok == 0xAA ? "ERROR" : "INFO"} #$number class=$cls state=$state msg="$msg"');
        pos += len;
      } else {
